                                    ...
                                    if (response.length !== 0) {
                                        if (DataExportDate > response[0].DataExportDate) {
                                            models.sequelize.query("SELECT * FROM P_InvoiceDetails where DataExportDate > " + "'" + response[0].DataExportDate.toISOString() + "'", { type: models.sequelize.QueryTypes.SELECT })
                                                .then(data2 => {
                                                    models.sequelize.query("SELECT COUNT(*)N'Count' FROM P_InvoiceDetails where DataExportDate > " + "'" + response[0].DataExportDate.toISOString() + "'", { type: models.sequelize.QueryTypes.SELECT })
                                                        .then(Countvalue => {
...
async function insertRecords(data, date) {
    ...
    models.InvoiceDetails.bulkCreate(data).then(async function (response) {
        ...
        models.sequelize.query("SELECT COUNT(*) N'MigrateCount' FROM InvoiceDetails where DataExportDate > " + "'" + date + "'", { type: models.sequelize.QueryTypes.SELECT })
            .then(migratedcount => {
