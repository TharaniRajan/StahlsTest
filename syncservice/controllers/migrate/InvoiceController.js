                                    ...
                                    if (response.length !== 0) {
                                        if (DataExportDate > response[0].DataExportDate) {
                                            ...
                                            models.sequelize.query("SELECT * FROM P_Invoices where DataExportDate > " + "'" + response[0].DataExportDate.toISOString() + "'", { type: models.sequelize.QueryTypes.SELECT })
                                                .then(data2 => {
                                                    models.sequelize.query("SELECT COUNT(*)N'Count' FROM P_Invoices where DataExportDate > " + "'" + response[0].DataExportDate.toISOString() + "'", { type: models.sequelize.QueryTypes.SELECT })
                                                        .then(Countvalue => {
...
async function insertRecords(data, date) {
    ...
    models.Invoices.bulkCreate(data).then(async function (response) {
        ...
        models.sequelize.query("SELECT COUNT(*) N'MigrateCount' FROM Invoices where DataExportDate > " + "'" + date + "'", { type: models.sequelize.QueryTypes.SELECT })
            .then(migratedcount => {
