...
module.exports.getpriorityticket = function (Days,CompanyCode, callback) {
    var listoftickets;
    sequelize.query("SELECT * FROM Tickets WHERE DATEDIFF(day,DATEADD(hh, -4 ,GETDATE()),Date)<=$dayvalue AND Type = 'Urgent' AND organizationUuid = $companyCode", {bind:{dayvalue: Days, companyCode: CompanyCode}, type: sequelize.QueryTypes.SELECT })
    .then(Ticketlist => {
