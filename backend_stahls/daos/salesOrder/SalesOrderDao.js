...
module.exports.getAllSalesOrders = function (pageNumber, pageSize, sortLabel, sortDirection, search, location, ticketCount, status, styleOption, callback) {
    ...
    var searchQuery = '';
    if (search !== '') {
        searchQuery = "( OrderNumber LIKE '%" + search + "%' OR CompanyCode LIKE '%" + search +
            "%' OR PONumber LIKE '%" + search + "%' OR OrderDate LIKE '%" + search +
            "%' OR RequiredDate LIKE '%" + search + "%' OR ForecastFinish LIKE '%" + search +
            "%' OR Tickets LIKE '%" + search + "%' OR StatusName LIKE '%" + search + 
            "%' OR Comments7 LIKE '%"+ search + "%' )";
    }
...
module.exports.getSalesOrderByStatusName = function (pageNumber, pageSize, sortLabel, sortDirection, search, location,
    ...
    //     })
    // } else
    //  if (location.length !== 0 && ticketCount.length !== 0 && status.length !== 0) {
    ...
    // } else {
    if (location.length !== 0 && ticketCount.length !== 0 && status.length !== 0 && styleOption.length != 0) {
        whereQuery = " WHERE StatusName=:statusName AND CompanyCode IN (:code) AND Tickets IN (:ticket) AND StatusName IN (:status) AND Comments7 IN (:styleOption)"
    } //two filter values with location
    else if (location.length !== 0 && ((ticketCount.length !== 0 && status.length === 0 && styleOption.length === 0) ||
            (status.length !== 0 && ticketCount.length === 0 && styleOption.length === 0) ||
            (status.length == 0 && ticketCount.length == 0 && styleOption.length !== 0))) {
        if (ticketCount.length != 0) {
            whereQuery = "WHERE StatusName=:statusName AND  CompanyCode IN (:code) AND Tickets IN (:ticket)"
        } else if (status.length != 0) {
            whereQuery = "WHERE StatusName=:statusName AND  CompanyCode IN (:code) AND StatusName IN (:status)"
        } else {
            whereQuery = "WHERE StatusName=:statusName AND  CompanyCode IN (:code) AND Comments7 IN (:styleOption)"
        }
    } // three filter values with location
    else if (location.length !== 0 && ((ticketCount.length !== 0 && status.length !== 0 && styleOption.length === 0) ||
            (status.length !== 0 && ticketCount.length === 0 && styleOption.length !== 0) ||
        ...
        console.log('entering into location there way filter', location, ticketCount, status);
        if (ticketCount.length != 0 && status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  CompanyCode IN (:code) AND Tickets IN (:ticket) AND StatusName IN (:status)"
        } else if (status.length != 0 && styleOption.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  CompanyCode IN (:code) AND StatusName IN (:status) AND Comments7 IN (:styleOption)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND  CompanyCode IN (:code) AND Tickets IN (:ticket) AND Comments7 IN (:styleOption)"
        }
    } //two filter values with Tickets
    else if (ticketCount.length !== 0 && ((location.length !== 0 && status.length === 0 && styleOption.length === 0) ||
            (status.length !== 0 && location.length === 0 && styleOption.length === 0) ||
        ...
        console.log('entering into ticket counts two way filter', location, ticketCount, status);
        if (location.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  Tickets IN (:ticket) AND CompanyCode IN (:code)"
        } else if (status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  Tickets IN (:ticket) AND StatusName IN (:status)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND  Tickets IN (:ticket) AND Comments7 IN (:styleOption)"
        }
    } // three filter values with Tickets
    else if (ticketCount.length !== 0 && ((location.length !== 0 && status.length !== 0 && styleOption.length === 0) ||
            (status.length !== 0 && location.length === 0 && styleOption.length !== 0) ||
        ...
        console.log('entering into ticket counts three way filter', location, ticketCount, status);
        if (location.length != 0 && status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  Tickets IN (:ticket) AND CompanyCode IN (:code) AND StatusName IN (:status)"
        } else if (status.length != 0 && styleOption.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  Tickets IN (:ticket) AND StatusName IN (:status) AND Comments7 IN (:styleOption)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND  Tickets IN (:ticket) AND Tickets IN (:ticket) AND Comments7 IN (:styleOption)"
        }
    } //two filter values with statusName
    else if (status.length !== 0 && ((ticketCount.length !== 0 && location.length === 0 && styleOption.length === 0) ||
            (location.length !== 0 && ticketCount.length === 0 && styleOption.length === 0) ||
        ...
        console.log('entering into status two way filter', location, ticketCount, status);
        if (ticketCount.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  StatusName IN (:status) AND Tickets IN (:ticket)"
        } else if (location.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  StatusName IN (:status) AND CompanyCode IN (:code)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND  StatusName IN (:status) AND Comments7 IN (:styleOption)"
        }
    } // three filter values with statusName
    else if (status.length !== 0 && ((ticketCount.length !== 0 && location.length !== 0 && styleOption.length === 0) ||
            (location.length !== 0 && ticketCount.length === 0 && styleOption.length !== 0) ||
        ...
        console.log('entering into status name two way filter', location, ticketCount, status);
        if (ticketCount.length != 0 && location.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  StatusName IN (:status) AND Tickets IN (:ticket) AND CompanyCode IN (:code)"
        } else if (location.length != 0 && styleOption.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  StatusName IN (:status) AND CompanyCode IN (:code) AND Comments7 IN (:styleOption)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND  StatusName IN (:status) AND Tickets IN (:ticket) AND Comments7 IN (:styleOption)"
        }
    } //two filter values with style option
    else if (styleOption.length !== 0 && ((ticketCount.length !== 0 && status.length === 0 && location.length === 0) ||
            (status.length !== 0 && ticketCount.length === 0 && location.length === 0) ||
        ...
        console.log('entering into style option two way filter', location, ticketCount, status);
        if (ticketCount.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  Comments7 IN (:styleOption) AND Tickets IN (:ticket)"
        } else if (status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  Comments7 IN (:styleOption) AND StatusName IN (:status)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND  Comments7 IN (:styleOption) AND CompanyCode IN (:code)"
        }
    } // three filter values with location
    else if (styleOption.length !== 0 && ((ticketCount.length !== 0 && status.length !== 0 && location.length === 0) ||
            (status.length !== 0 && ticketCount.length === 0 && location.length !== 0) ||
        ...
        console.log('entering into style option there way filter', location, ticketCount, status);
        if (ticketCount.length != 0 && status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  Comments7 IN (:styleOption) AND Tickets IN (:ticket) AND StatusName IN (:status)"
        } else if (status.length != 0 && location.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND  Comments7 IN (:styleOption) AND StatusName IN (:status) AND CompanyCode IN (:code)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND  Comments7 IN (:styleOption) AND Tickets IN (:ticket) AND CompanyCode IN (:code)"
        }
    } else if (location.length !== 0 && ticketCount.length === 0 && status.length === 0 && styleOption.length === 0) {
        whereQuery = "WHERE  StatusName=:statusName AND  CompanyCode IN (:code)";
    } else if (ticketCount.length !== 0 && location.length === 0 && status.length === 0 && styleOption.length === 0) {
        whereQuery = "WHERE  StatusName=:statusName AND  Tickets IN (:ticket)";
    } else if (status.length !== 0 && ticketCount.length === 0 && location.length === 0 && styleOption.length === 0) {
        whereQuery = "WHERE  StatusName=:statusName AND  StatusName IN (:status)";
    } else if (styleOption.length !== 0 && ticketCount.length === 0 && status.length === 0 && location.length === 0) {
        whereQuery = "WHERE  StatusName=:statusName AND  Comments7 IN (:styleOption)";
    } else {
        // callback([])
        whereQuery = "WHERE StatusName=:statusName"
        console.log('nothing if condition is called')
    }
    var searchQuery = '';
    if (search !== '') {
...
module.exports.getSalesOrderByStatusNameCompanyCode = function (pageNumber, pageSize, sortLabel, sortDirection, search, location,
    ...
    console.log('affset and fetch values are ---- ', pageNumber, pageSize, offsetValue)
    if (location.length !== 0 && ticketCount.length !== 0 && status.length !== 0 && styleOption.length != 0) {
        whereQuery = " WHERE StatusName=:statusName AND CompanyCode = :companyCode AND CompanyCode IN (:code) AND Tickets IN (:ticket) AND StatusName IN (:status) AND Comments7 IN (:styleOption)"
    } //two filter values with location
    else if (location.length !== 0 && ((ticketCount.length !== 0 && status.length === 0 && styleOption.length === 0) ||
            (status.length !== 0 && ticketCount.length === 0 && styleOption.length === 0) ||
            (status.length == 0 && ticketCount.length == 0 && styleOption.length !== 0))) {
        if (ticketCount.length != 0) {
            whereQuery = "WHERE StatusName=:statusName AND CompanyCode = :companyCode AND  CompanyCode IN (:code) AND Tickets IN (:ticket)"
        } else if (status.length != 0) {
            whereQuery = "WHERE StatusName=:statusName AND CompanyCode = :companyCode AND  CompanyCode IN (:code) AND StatusName IN (:status)"
        } else {
            whereQuery = "WHERE StatusName=:statusName AND CompanyCode = :companyCode AND  CompanyCode IN (:code) AND Comments7 IN (:styleOption)"
        }
    } // three filter values with location
    else if (location.length !== 0 && ((ticketCount.length !== 0 && status.length !== 0 && styleOption.length === 0) ||
            (status.length !== 0 && ticketCount.length === 0 && styleOption.length !== 0) ||
        ...
        console.log('entering into location there way filter', location, ticketCount, status);
        if (ticketCount.length != 0 && status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  CompanyCode IN (:code) AND Tickets IN (:ticket) AND StatusName IN (:status)"
        } else if (status.length != 0 && styleOption.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  CompanyCode IN (:code) AND StatusName IN (:status) AND Comments7 IN (:styleOption)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  CompanyCode IN (:code) AND Tickets IN (:ticket) AND Comments7 IN (:styleOption)"
        }
    } //two filter values with Tickets
    else if (ticketCount.length !== 0 && ((location.length !== 0 && status.length === 0 && styleOption.length === 0) ||
            (status.length !== 0 && location.length === 0 && styleOption.length === 0) ||
        ...
        console.log('entering into ticket counts two way filter', location, ticketCount, status);
        if (location.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Tickets IN (:ticket) AND CompanyCode IN (:code)"
        } else if (status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Tickets IN (:ticket) AND StatusName IN (:status)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Tickets IN (:ticket) AND Comments7 IN (:styleOption)"
        }
    } // three filter values with Tickets
    else if (ticketCount.length !== 0 && ((location.length !== 0 && status.length !== 0 && styleOption.length === 0) ||
            (status.length !== 0 && location.length === 0 && styleOption.length !== 0) ||
        ...
        console.log('entering into ticket counts three way filter', location, ticketCount, status);
        if (location.length != 0 && status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Tickets IN (:ticket) AND CompanyCode IN (:code) AND StatusName IN (:status)"
        } else if (status.length != 0 && styleOption.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Tickets IN (:ticket) AND StatusName IN (:status) AND Comments7 IN (:styleOption)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Tickets IN (:ticket) AND Tickets IN (:ticket) AND Comments7 IN (:styleOption)"
        }
    } //two filter values with statusName
    else if (status.length !== 0 && ((ticketCount.length !== 0 && location.length === 0 && styleOption.length === 0) ||
            (location.length !== 0 && ticketCount.length === 0 && styleOption.length === 0) ||
        ...
        console.log('entering into status two way filter', location, ticketCount, status);
        if (ticketCount.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  StatusName IN (:status) AND Tickets IN (:ticket)"
        } else if (location.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  StatusName IN (:status) AND CompanyCode IN (:code)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  StatusName IN (:status) AND Comments7 IN (:styleOption)"
        }
    } // three filter values with statusName
    else if (status.length !== 0 && ((ticketCount.length !== 0 && location.length !== 0 && styleOption.length === 0) ||
            (location.length !== 0 && ticketCount.length === 0 && styleOption.length !== 0) ||
        ...
        console.log('entering into status name two way filter', location, ticketCount, status);
        if (ticketCount.length != 0 && location.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  StatusName IN (:status) AND Tickets IN (:ticket) AND CompanyCode IN (:code)"
        } else if (location.length != 0 && styleOption.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  StatusName IN (:status) AND CompanyCode IN (:code) AND Comments7 IN (:styleOption)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  StatusName IN (:status) AND Tickets IN (:ticket) AND Comments7 IN (:styleOption)"
        }
    } //two filter values with style option
    else if (styleOption.length !== 0 && ((ticketCount.length !== 0 && status.length === 0 && location.length === 0) ||
            (status.length !== 0 && ticketCount.length === 0 && location.length === 0) ||
        ...
        console.log('entering into style option two way filter', location, ticketCount, status);
        if (ticketCount.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Comments7 IN (:styleOption) AND Tickets IN (:ticket)"
        } else if (status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Comments7 IN (:styleOption) AND StatusName IN (:status)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Comments7 IN (:styleOption) AND CompanyCode IN (:code)"
        }
    } // three filter values with location
    else if (styleOption.length !== 0 && ((ticketCount.length !== 0 && status.length !== 0 && location.length === 0) ||
            (status.length !== 0 && ticketCount.length === 0 && location.length !== 0) ||
        ...
        console.log('entering into style option there way filter', location, ticketCount, status);
        if (ticketCount.length != 0 && status.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Comments7 IN (:styleOption) AND Tickets IN (:ticket) AND StatusName IN (:status)"
        } else if (status.length != 0 && location.length != 0) {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Comments7 IN (:styleOption) AND StatusName IN (:status) AND CompanyCode IN (:code)"
        } else {
            whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Comments7 IN (:styleOption) AND Tickets IN (:ticket) AND CompanyCode IN (:code)"
        }
    } else if (location.length !== 0 && ticketCount.length === 0 && status.length === 0 && styleOption.length === 0) {
        whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  CompanyCode IN (:code)";
    } else if (ticketCount.length !== 0 && location.length === 0 && status.length === 0 && styleOption.length === 0) {
        whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Tickets IN (:ticket)";
    } else if (status.length !== 0 && ticketCount.length === 0 && location.length === 0 && styleOption.length === 0) {
        whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  StatusName IN (:status)";
    } else if (styleOption.length !== 0 && ticketCount.length === 0 && status.length === 0 && location.length === 0) {
        whereQuery = "WHERE  StatusName=:statusName AND CompanyCode = :companyCode AND  Comments7 IN (:styleOption)";
    } else {
        // callback([])
        whereQuery = "WHERE StatusName=:statusName AND CompanyCode = :companyCode"
        console.log('nothing if condition is called')
    }
    var searchQuery = ''
    if (search !== '') {
